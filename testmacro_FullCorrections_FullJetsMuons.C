                                             ...
                                             const std::string &muPOGDir   = "/afs/cern.ch/user/n/nbostan/new_CMS/CMSSW_14_0_18/src/RoccoR/data/POG/MUO/")
{
    ...
    TH2F *h_mass_vs_pt = new TH2F("h_mass_vs_pt", "Dimuon invariant mass vs p_{T}^{#mu#mu};M_{#mu#mu} [GeV];p_{T}^{#mu#mu} [GeV]", 80, 70, 110, 100, 0, 500);
    TH2F *h_dijetMass_vs_pt = new TH2F("h_dijetMass_vs_pt", "Dijet invariant mass vs p_{T}^{jj};M_{jj} [GeV];p_{T}^{jj} [GeV]", 100, 0, 2000, 100, 0, 1000);
    ...
    if (isMC && hasGenPt) Muon_genPt = new TTreeReaderArray<Float_t>(reader, "Muon_genPt");
    if (isMC && hasPileup) Pileup_nTrueInt = new TTreeReaderValue<Float_t>(reader, "Pileup_nTrueInt");
    ...
    while(reader.Next()) {
        ...
        for (int p = 0; p < *nMuon; p++) {
            ...
            if (isMC) {
                ...
                sf_tag = (genPt_tag>0) ? rc.kSpreadMC(Muon_charge[tagIdx], Muon_pt[tagIdx], Muon_eta[tagIdx], Muon_phi[tagIdx], genPt_tag, 0, 0)
                                       : rc.kScaleMC(Muon_charge[tagIdx], Muon_pt[tagIdx], Muon_eta[tagIdx], Muon_phi[tagIdx], 0, 0);
                sf_probe = (genPt_probe>0) ? rc.kSpreadMC(Muon_charge[p], Muon_pt[p], Muon_eta[p], Muon_phi[p], genPt_probe, 0, 0)
            ...
            if (!std::isfinite(totalWeight)) continue;
            ...
            if (!std::isfinite(dimuon.M()) || !std::isfinite(dimuon.Pt())) continue;
    ...
    std::cout << "Events passing HLT + nMuon >= 2 cuts: " << nEventPassed << std::endl;
