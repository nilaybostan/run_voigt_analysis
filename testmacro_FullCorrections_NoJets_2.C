...
void DrawComparisonWithRatio(TH1D* hUnweighted, TH1D* hWeighted, TH1D* hRatio, const std::string& name, const std::string& xTitle) {
    ...
    // Draw Weighted first (W red, U blue)
    ...
    hRatio->SetStats(kFALSE);  // hide ratio statbox
...
)
{
    ...
    TH1D* hMassRatio = (TH1D*) hMassWeighted->Clone("hMassRatio");
    hMassRatio->Reset();
    TH1D* hPtRatio   = (TH1D*) hProbePtWeighted->Clone("hPtRatio");
    hPtRatio->Reset();
    TH1D* hEtaRatio  = (TH1D*) hProbeEtaWeighted->Clone("hEtaRatio");
    hEtaRatio->Reset();
    ...
    hMassRatio->Sumw2();
    hMassRatio->Divide(hMassWeighted, hMassUnweighted, 1.0, 1.0, "B");

    hPtRatio->Sumw2();
    hPtRatio->Divide(hProbePtWeighted, hProbePtUnweighted, 1.0, 1.0, "B");

    hEtaRatio->Sumw2();
    hEtaRatio->Divide(hProbeEtaWeighted, hProbeEtaUnweighted, 1.0, 1.0, "B");
    ...
    std::cout<<"Finished Macro, output saved to "<<outFileName<<std::endl;
}
